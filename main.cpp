...
int main(int argc, char* argv[]) {
    ...
    ARPSpoofingInterface arpSpoofingManager;

    if (argc == 2) {
        std::cout << "INFO: broadcast ARP-Spoofing..." << std::endl;
        arpSpoofingManager = BroadcastARPSpoofing(argv[1]);
    } else {
        ...
        arpSpoofingManager = SessionARPSpoofing(argv[1], pairs);
    }

    arpSpoofingManager->buildSessions();
    arpSpoofingManager->startARPSpoofing(ARPSpoofingInterface::UNLIMITED);
    return 0;
}
